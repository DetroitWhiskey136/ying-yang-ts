...
import { Database, Structure, Util } from '..';

export declare interface WarningManager {
  member: GuildMember;
  guild: Guild;
  cache: Collection<string, Structure.Warning>;
}
...
export class WarningManager {
  ...
  // #region Methods
  private init() {
    const member = this.member.client.bot.database.members.get(this.member.id);

    if (member && member.guilds) {
      const guildData = member.guilds[this.guild.id];

      const warnings = guildData
        ? guildData.warnings
        : (this.cache = new Collection());
      ...
      warnings.forEach((warning: Database.Models.Member.IMemberWarning) => this.cache.set(
        warning.id,
        new Structure.Warning(this.member.client, warning, this.member),
      ));
    }
  }
  ...
  public add(reason: string, points: number, moderator: GuildMember | null) {
    ...
    let id = Util.Strings.makeID(18);
    while (this.cache && this.cache.has(id)) {
      id = Util.Strings.makeID(18);
    }
  ...
  public remove(id: string) {
    const db = this.member.client.bot.database;
    let { warnings } = db.members.model.get(this.member.id).guilds[
      this.guild.id
    ];
    ...
    const warning = warnings.filter(
      (warning: Database.Models.Member.IMemberWarning) => warning.id === id,
    );
  ...
  private update(id: string, data?: Database.Models.Member.IMemberWarning) {
    const db = this.member.client.bot.database;
    const { warnings } = db.members.get(this.member.id).guilds[this.guild.id];
