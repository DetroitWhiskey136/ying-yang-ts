import { Message, TextChannel, DMChannel, NewsChannel, ColorResolvable } from 'discord.js';
...
export default class MessageEvent extends Event {
  ...
  private getPrefix (prefix: string, bot: BotClient, { client, content, guild }: Message): string | null {
  ...
  run (bot: BotClient, client: DiscordClient, message: Message) {
    ...
    const usedPrefix = this.getPrefix(prefix, bot, message);
    ...
    if (mentioned && !usedPrefix) return this.createEmbed(channel, 'BLUE', StringUtil.hasPlaceholder(MESSAGES.PREFIX, '{prefix}', `\`${prefix}\``));
    ...
    if (mentioned && !command) return this.createEmbed(channel, 'BLUE', StringUtil.hasPlaceholder(MESSAGES.PREFIX, '{prefix}', `\`${prefix}\``));

    if (!command || !usedPrefix) return;

    const params = { bot, args, message, prefix: guild?.prefix || '/', query: args.join(' ') };
    return command._run(new CommandContext(params));
  }
}
